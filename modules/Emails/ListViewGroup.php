...
if (isset($_REQUEST['query'])) {
    ...
    if (isset($email_type) && $email_type != "") {
        $whereClauses['emails.type'] = "emails.type = '".DBManagerFactory::getInstance()->quote($email_type)."'";
    }
    if (isset($assigned_to) && $assigned_to != "") {
        $whereClauses['emails.assigned_user_id'] = "emails.assigned_user_id = '".DBManagerFactory::getInstance()->quote($assigned_to)."'";
    }
    if (isset($status) && $status != "") {
        $whereClauses['emails.status'] = "emails.status = '".DBManagerFactory::getInstance()->quote($status)."'";
    }
    if (isset($name) && $name != "") {
        $whereClauses['emails.name'] = "emails.name like '".DBManagerFactory::getInstance()->quote($name)."%'";
    }
