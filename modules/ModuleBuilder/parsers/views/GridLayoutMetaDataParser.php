...
class GridLayoutMetaDataParser extends AbstractMetaDataParser implements MetaDataParserInterface
{
    ...
    */
            ...
            foreach ($panel as $rowID => $row) {
                ...
                foreach ($row as $colID => $fieldname) {
                    ...
                    if (is_array($fieldname)) {
                        ...
                        continue;
                    } elseif (!isset($fielddefs[$fieldname]) && $fieldname != $this->FILLER['name']) {
                        continue;
                    }
                    ...
                    //Use the previous viewdef if this field was on it.
                    elseif (isset($previousViewDef[$fieldname])) {
     ...
     */
    public function getNewRowItem($source, $fielddef)
    {
        ...
        if (isset($def['studio'])) {
            if (is_array($def['studio'])) {
                if (!empty($def['studio'][$this->_view]) && $def['studio'][$this->_view] == "required") {
                    return true;
                } elseif (!empty($def['studio']['required']) && $def['studio']['required'] == true) {
                    return true;
                }
            } elseif ($def['studio'] == "required") {
                return true;
            }
        }
        return false;
