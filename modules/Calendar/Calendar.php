...
class Calendar
{
    ...
    public $activityList = array(
        "FP_events" => array("showCompleted" => true, "start" => "date_start", "end" => "date_end"),
    ...
    // Current view
    public $view = 'agendaWeek';
    // Calendar style (basic or advanced)
    public $style;
    // If is displayed in dashlet
    public $dashlet = false;
    // Current date
    public $date_time;
    ...
    // Time step of each slot in minutes
    public $time_step = 60;

    // Array of activities objects
    public $acts_arr = array();
    // Array of activities data to be displayed
    public $items = array();
    // Ids of users for shared view
    public $shared_ids = array();

    // Entire 24h day count of slots
    public $cells_per_day;
    // Start timestamp of calendar grid
    public $grid_start_ts;

    // Working day start time in format '11:00
    public $day_start_time;
    // Working day end time in format '11:00'
    public $day_end_time;
    // First slot of working day
    public $scroll_slot;
    // Count of slots in a working day
    public $celcount;
    ...
    function __construct($view = "agendaWeek", $time_arr = array())
    {
        ...
        $current_date_db = $date_arr['year'] . "-" . str_pad($date_arr['month'], 2, "0",
                STR_PAD_LEFT) . "-" . str_pad($date_arr['day'], 2, "0", STR_PAD_LEFT);
            ...
            foreach ($acts as $act) {
                ...
                if (isset($this->activityList[$act->sugar_bean->module_name]['start']) && !empty($this->activityList[$act->sugar_bean->module_name]['start'])) {
                    $item = array_merge($item, CalendarUtils::get_time_data($act->sugar_bean,
                        $this->activityList[$act->sugar_bean->module_name]['start'],
                        $this->activityList[$act->sugar_bean->module_name]['end']));
                } else {
                    $item = array_merge($item, CalendarUtils::get_time_data($act->sugar_bean));
                }
    ...
    public function add_activities($user, $type = 'sugar')
    {
        ...
        if ($type == 'vfb') {
            ...
            $acts_arr = CalendarActivity::get_activities($this->activityList, $user->id, $this->show_tasks,
