...
class campaign_charts
{
    ...
    function campaign_response_by_activity_type(
        $datay = array(),
        $targets = array(),
        $campaign_id = null,
        ...
        if ($campaign_id) {
            ...
            if (!file_exists($xmlFile) || $refresh == true) {
                ...
                foreach ($datay as $key => $translation) {
                    ...
                    if (is_array($leadSourceArr[$key]) && isset($leadSourceArr[$key]['row_total']) &&
                        $leadSourceArr[$key]['row_total'] > 100
                    ) {
                    ...
                    foreach ($targets as $outcome => $outcome_translation) {
                        ...
                        if (isset($targeted) && isset($targeted[$outcome]) && !empty($targeted[$outcome])) {
                            $alttext = $targets[$outcome] . ': ' .
                                $mod_strings['LBL_TARGETED'] . ' ' . $targeted[$outcome] . ', ' .
                                $mod_strings['LBL_TOTAL_TARGETED'] . ' ' . $total_targeted . ".";
                        }
                        if ($key != 'targeted') {
                            $hits = (isset($leadSourceArr[$key][$outcome]) &&
                                is_array($leadSourceArr[$key][$outcome]) &&
                                is_array($leadSourceArr[$key][$outcome]['hits'])) ?
                                array_sum($leadSourceArr[$key][$outcome]['hits']) : 0;
                            $alttext .= " $translation " . $hits;
                        }
                        $count = (isset($leadSourceArr[$key][$outcome]) &&
                            is_array($leadSourceArr[$key][$outcome]) &&
                            is_array($leadSourceArr[$key][$outcome]['total'])) ?
                            array_sum($leadSourceArr[$key][$outcome]['total']) : 0;
                ...
                // Since this isn't a standard report chart (with report defs),
                // set the group_by manually so the chart bars show properly
    ...
    function campaign_response_roi(
        ...
        global $app_strings, $mod_strings, $current_module_strings, $charset, $lang,
               $app_list_strings, $current_language, $sugar_config;
        ...
        if ((!file_exists($cache_file_name) || $refresh == true) && $campaign_id) {
            ...
            $opp_count = 0;
            $opp_query = "select count(*) opp_count,sum(" . db_convert("amount_usdollar", "IFNULL",
                    array(0)) . ")  total_value";
    ...
    function campaign_response_roi_data(
        $datay = array(),
        $targets = array(),
        $campaign_id = null,
        $cache_file_name = 'a_file',
