...
class Release extends SugarBean
{
    ...
    public function get_list_view_data()
    {
        ...
        if (isset($app_list_strings['release_status_dom'][$this->status])) {
            $status = $app_list_strings['release_status_dom'][$this->status];
        } else {
            LoggerManager::getLogger()->warn('Status is not defined in app_list_strings[release_status_dom][' . $this->status. ']');
        }
        
        $temp_array['ENCODED_STATUS'] = $status;
