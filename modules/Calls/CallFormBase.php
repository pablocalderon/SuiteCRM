...
EOQ;
    ...
    function handleSave($prefix, $redirect = true, $useRequired = false)
    {
        ...
        if (!isset($_POST['email_reminder_checked']) || (isset($_POST['email_reminder_checked']) &&
        ...
        if (!empty($_POST[$prefix . 'time_hour_start']) && empty($_POST[$prefix . 'time_start'])) {
            $_POST[$prefix . 'time_start'] = $_POST[$prefix . 'time_hour_start'] .
                $time_separator . $_POST[$prefix . 'time_minute_start'];
        }
        ...
        if (empty($focus->id) && !empty($_REQUEST['return_module']) && $_REQUEST['return_module'] == 'Calls' &&
            !empty($_REQUEST['return_action']) && $_REQUEST['return_action'] === 'DetailView'
        ) {
            // If return action is set to detail view and return module to call, then this is from the long form,
            // Do not add the assigned user (only the current user)
            // The current user is already added to UI and
            // We want to give the current user the option of opting out of meeting.
