...
class ModuleScanner
{
    ...
    /**
     *This function will scan the Manifest for disabled actions specified in
     * $GLOBALS['sugar_config']['moduleInstaller']['disableActions']
     *if $GLOBALS['sugar_config']['moduleInstaller']['disableRestrictedCopy'] is set to false or not set it will call
     * on scanCopy to ensure that it is not overriding files
     */
    ...
    public function displayIssues($package = 'Package')
    {
        echo '<h2>' . str_replace(
            ...
            translate('ML_SUGAR_KB') . '</a>.' .
            '<br>' . translate('ML_AVAIL_RESTRICTION') .
            ' <a href=" http://developers.sugarcrm.com/wordpress/2009/08/14/module-loader-restrictions/">' .
            translate('ML_SUGAR_DZ') . '</a>.<br><br>';
        ...
        foreach ($this->issues as $type => $issues) {
            ...
            foreach ($issues as $file => $issue) {
                ...
                echo '<div style="position:relative;left:10px"><b>' .
                    $file . '</b></div><div style="position:relative;left:20px">';
                if (is_array($issue)) {
                    foreach ($issue as $i) {
                        echo "$i<br>";
                    }
